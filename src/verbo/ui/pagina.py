...
from verbo.core.busca import buscar_versiculos
from verbo.core.resposta import gerar_resposta, continuar_conversa
...
from verbo.core.util import resumir_texto, localizar_capitulo
from verbo.ui.cache import carregar_capitulos, carregar_versiculos
from verbo.ui.plano_livre import renderizar_seletor_livro_capitulo
...
def render():
    ...
    def _renderizar_busca_semantica(capitulos_leitura):
        ...
        if not ultima_busca:
            ...
            versiculo_dia = obter_versiculo_do_dia(carregar_versiculos(), data_hoje)
            ...
            for i, v in enumerate(versiculos_visiveis):
                with st.container(border=True, key=f"versiculo_card_{i}"):
                    ...
                    if st.button(
                        ...
                        idx_capitulo = localizar_capitulo(capitulos_leitura, v["referencia"])
                        if idx_capitulo is not None:
                            st.session_state.capitulo_aberto = idx_capitulo
                            st.rerun()
                    st.write(resumir_texto(v["texto"]))
    ...
    def _renderizar_sidebar_idle(capitulos_leitura):
        with st.container(key="sb_idle_bloco"):
            ...
            with st.container(border=True, key="sb_livros"):
                ...
                renderizar_seletor_livro_capitulo(capitulos_leitura)
